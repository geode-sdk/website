<script lang="ts">
    ...
    let formatted = derived(countup, ($countup) =>
        $countup.toLocaleString(undefined, { maximumFractionDigits: 0 }),
    );
...
<div>
    <Icon {icon} /><span class="countup" bind:this={number}>
        {$formatted}
    </span>{text}
</div>
