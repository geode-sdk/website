...
{#if newGDUpdate?.geodeStatus === "fully-broken"}
    <InfoBox type="error" solid={true}>
        A new update for Geometry Dash (version {newGDUpdate.newGDVersion}) was recently released, and because of that
        <em>Geode is currently broken</em>. We're working on fixing it, but please be patient!
...
{:else if newGDUpdate?.geodeStatus === "just-updated"}
    <InfoBox type="info" solid={true}>
        A new update for Geometry Dash (version {newGDUpdate.newGDVersion}) was recently released.
        <em>Geode now supports that version</em>, but most mods haven't been updated yet. Please wait for mod developers
